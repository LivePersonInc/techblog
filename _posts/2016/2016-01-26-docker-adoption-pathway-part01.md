...
Today, `docker` and `kubernetes` are the mainstream for containerized deployment, and it's what we are evaluating, so, we are going to refer to these technologies in this article, however, the insights are generic.  Let's now dig inside the process details:
